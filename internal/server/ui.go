...
async function load(){var r=await fetch(A+'/zones').then(function(r){return r.json()});items=r.zones||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.region||'').toLowerCase().includes(q)||(i.type||'').toLowerCase().includes(q)||(i.capacity||'').toLowerCase().includes(q)});
...
if(i.region)h+='<div class="item-sub">'+esc(i.region)+'</div>';
...
if(i.capacity)h+='<span>'+esc(i.capacity)+'</span>';
if(i.used_count)h+='<span>'+esc(i.used_count)+'</span>';
...
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/zones/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{name:"",region:"",type:"",capacity:"",used_count:"",status:"",notes:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' RANGELAND</h2>';
h+='<div class="fr"><label>Name *</label><input id="f-name" value="'+esc(i.name)+'"></div>';
h+='<div class="fr"><label>Region</label><input id="f-region" value="'+esc(i.region)+'"></div>';
h+='<div class="fr"><label>Type</label><input id="f-type" value="'+esc(i.type)+'"></div>';
h+='<div class="fr"><label>Capacity</label><input id="f-capacity" value="'+esc(i.capacity)+'"></div>';
h+='<div class="fr"><label>Used</label><input id="f-used-count" value="'+esc(i.used_count)+'"></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),region:document.getElementById('f-region').value.trim(),type:document.getElementById('f-type').value.trim(),capacity:document.getElementById('f-capacity').value.trim(),used_count:document.getElementById('f-used-count').value.trim(),status:document.getElementById('f-status').value.trim(),notes:document.getElementById('f-notes').value.trim()};
if(editId){await fetch(A+'/zones/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/zones',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
